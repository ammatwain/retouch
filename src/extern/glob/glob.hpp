...
public:
    ...
    {
        this->privPattern = aPattern;
        this->privCompiledPattern = Matcher::compile_pattern(this->privPattern);
    }

    bool match(const fs::path &name) {
        return std::regex_match(name.string(), this->privCompiledPattern);
    }
...
private:
    std::regex privCompiledPattern;
    std::string privPattern;
};
